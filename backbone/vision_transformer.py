...
# --- Constants and Utilities from vit_utils.py ---
...
def load_state_dict(checkpoint_path, use_ema=False):
    if checkpoint_path and os.path.isfile(checkpoint_path):
        ...
        # print("No checkpoint found at '{}'".format(checkpoint_path)) # Using print for simplicity, can re-add logging if needed
...
def load_pretrained(model, cfg=None, num_classes=1000, in_chans=3, filter_fn=None, img_size=224, num_frames=8,
    ...
    if cfg is None or 'url' not in cfg or not cfg['url']:
        warnings.warn("Pretrained model URL is invalid, using random initialization.")
        return
    ...
    if in_chans == 1:
        ...
        O, I, J, K = conv1_weight.shape
        if I != 3:
            warnings.warn('Deleting first conv (%s) from pretrained weights.' % conv1_name)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        # Classifier head (can be removed if only using for feature extraction)
    ...
    def forward_features(self, x, get_all=False, get_attn=False):
        ...
        if get_all:
            return x
        return x[:, 0] # Returns the class token embedding
    ...
    def get_last_selfattention(self, x):
        # This method assumes `prepare_tokens` exists or is handled.
        # For this consolidated version, `forward_features` handles token preparation.
        x = self.forward_features(x, get_all=True)
        # Assuming last block's attention is returned by get_attn in forward_features
        # This needs to be carefully handled if `get_attn` logic is for inference.
        # For simplicity, if you need attention, use forward_features(x, get_attn=True)
        raise NotImplementedError("This method requires specific attention extraction logic which is outside the core feature extraction.")